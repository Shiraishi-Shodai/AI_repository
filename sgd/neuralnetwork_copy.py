...
class NeuralNetwork():
    ...
    def fit(self, X_train, y_train):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            # すげての勾配を求め、一次元の配列に結合した後にL2ノルムを計算
            grad_all = []
            for key in self.params.keys():
                key_grad = self.get_grad(X_train, y_train, key)
                key_grad_norm2 = np.linalg.norm(key_grad, ord=2)
    ...
    def update(self):
        ...
        for key in self.params.keys():
            ...
            for batch_index in mini_batch_indexes:
                ...
                total_grad += grad # あるキーのミニバッチ勾配を加算
            
            avg_grad = total_grad / batch_size # ミニバッチの勾配をバッチサイズで平均化

            self.params[key] -= self.η * avg_grad # 重みを更新
