...
class NeuralNetwork():
    ...
    def backward(self):
        dz3 = 2 * (1 / self.y_train.shape[0]) * (self.z3 - self.y_train)
        du3 = dz3 * 1
        dw3 = self.z2.T @ du3
...
def main():
    X = np.linspace(5, 35, 500).reshape(-1, 1)
    y = (X - 1) * (X - 2.4) **2
    y_noise = y + np.random.uniform(-0.3, 0.3, 500).reshape(-1, 1)
    ...
    # print(X[:5], X_scaled[:5])
    # print(y_noise[:5], y_noise_scaled[:5])

    # fig = plt.figure(figsize=(12, 5))
    # ax1 = fig.add_subplot(121)
    # ax1.scatter(X_scaled, y_scaled)
    # ax1.set_xlabel("X")
    # ax1.set_ylabel("y")
    # ax1.set_title("ノイズなしデータ")

    # ax2 = fig.add_subplot(122)
    # ax2.scatter(X_scaled, y_noise_scaled)
    # ax2.set_xlabel("X")
    # ax2.set_ylabel("y_noise")
    # ax2.set_title("ノイズありデータ")

    # plt.savefig("original_test_data.png")

    nn = NeuralNetwork(input_size=X.shape[1], hidden_size1=10, hidden_size2=30, output_size=1)
    nn.training(X_train=X_scaled, y_train=y_noise_scaled, epochs=100, lr=0.008412713567839196)
    y_pred_scaled = nn.test(X_test=X_scaled)
    mse = MSE(y_pred_scaled, y_noise_scaled)

    print(f"平均二乗誤差：{mse}")

    y_pred = nn.test(X_scaled)
    ...
    ax2.scatter(X_scaled, y_noise_scaled, label="元データ")
    ax2.plot(X_scaled, y_pred, label="予測データ", c="red")
    ax2.grid()    
    ax2.legend()
    ax2.set_title("予測")
    ax2.set_xlabel("x")
    ...
    plt.savefig("test_data_pred.png")
