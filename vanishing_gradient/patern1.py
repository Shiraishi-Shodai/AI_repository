# https://chatgpt.com/share/6842fcb1-25b0-8008-af62-5e17ced344af
...
class NeuralNetwork():
    ...
    def __init__(self, data_len, input_dim=1, hidden1=2, hidden2=3, hidden3=3, out_dim=1, epochs=1000, lr=0.001):
        ...
        self.hidden3 = hidden3  # 3層目のニューロンの個数
        self.out_dim = out_dim  # 4層目のニューロンの個数
        self.epochs = epochs
        self.lr = lr
    ...
    def initialize_parameters(self):
        ...
        fan_in = self.hidden2
        scale3 = np.sqrt(2.0 / fan_in)

        fan_in = self.hidden3
        scale4 = np.sqrt(2.0 / fan_in)


        self.params = {
            "w1": np.random.normal(loc=0, scale=scale1, size=(self.input_dim, self.hidden1)),
            "b1": np.zeros((1, self.hidden1)),
            "w2": np.random.normal(loc=0, scale=scale2, size=(self.hidden1, self.hidden2)),
            "b2": np.zeros((1, self.hidden2)),
            "w3": np.random.normal(loc=0, scale=scale3, size=(self.hidden2, self.hidden3)),
            "b3": np.zeros((1, self.hidden3)),
            "w4": np.random.normal(loc=0, scale=scale4, size=(self.hidden3, self.out_dim)),
            "b4": np.zeros((1, self.out_dim))
        }
    ...
    def foward(self, X):
        ...
        self.u3 = self.z2 @ self.params["w3"] + self.params["b3"]
        self.z3 = self.ReLU(self.u3)

        self.u4 = self.z3 @ self.params["w4"] + self.params["b4"] # (50, 1)
        self.z4 = self. u4 # (50, 1)

        return self.z4 # 目的変数の形に合わせる

    
    def backward(self, y_pred,  y):
        z4 = y_pred

        dz4 = 1/self.data_len * (z4 - y) # (50, 1)
        du4 = dz4 * 1 # (50, 1)
        dw4 = self.z3.T @ du4
        db4 = np.sum(du4, axis=0)

        dz3 = du4 @ self.params["w4"].T
        du3 = self.dReLU(self.u3, dz3)
...
def main():
    df = pd.read_csv("~/Documents/MatsuoLabo/knowledge/back_propagation/exe11.csv")
    X = df["x"].to_numpy().reshape(-1, 1)
    y = df["y"].to_numpy().reshape(-1, 1)

    nn = NeuralNetwork(data_len=X.shape[0], input_dim=X.shape[1], hidden1=5, hidden2=14, hidden3=10, out_dim=1, epochs=3000, lr=0.000452)
